...
class c_listener_entity : public c_entity_listener, c_sdk {
private:
...
public:
	...
	virtual void store() override {
		DEBUG_LOG("  [+] Create entity listener \n\n");

		__interfaces->entity_list->add_listener(this);
	}

	virtual void destroy() override {
		__interfaces->entity_list->remove_listener(this);
	}
};

#ifdef _DEBUG
inline auto __entlistener = std::make_unique<c_listener_entity>();
#else
CREATE_DUMMY_PTR(c_listener_entity);
DECLARE_XORED_PTR(c_listener_entity, GET_XOR_KEYUI32);

#define __entlistener XORED_PTR(c_listener_entity)
#endif
