...
public:
	...
	INLINE void set_value(const std::uint32_t& handle) {
		value = handle;
	}

	INLINE c_base_handle(std::uint32_t handle) : value(handle) {};
	INLINE c_base_handle() : value(0) {}
...
static INLINE int calc_new_allocation_count(int alloc_count, int grow_size, int nNewSize, int bytes_item) {
	if (grow_size)
		alloc_count = ((1 + ((nNewSize - 1) / grow_size)) * grow_size);
	else
	{
		if (!alloc_count)
			alloc_count = (31 + bytes_item) / bytes_item;

		while (alloc_count < nNewSize)
			alloc_count *= 2;
	}
...
public:
	...
	void shift_elements_left(int elem, int num = 1) {
		int num_to_move = size - elem - num;
		if ((num_to_move > 0) && (num > 0))
			std::memmove(&element(elem), &element(elem + num), num_to_move * sizeof(T));
	}
