...
public:
	...
	INLINE c_voice_communication_data get_data() const {
...
public:
	...
	INLINE float get_float() const {
		std::uint32_t xored = *(std::uintptr_t*)(&parent->float_value) ^ (std::uintptr_t)this;
		return *(float*)&xored;
	}

	INLINE int get_int() const {
		return parent->int_value ^ (std::uintptr_t)this;
	}
...
public:
	...
	virtual std::uint64_t get_uint64(const char* key_name = NULL, std::uint64_t default_value = 0) const = 0;
	...
	virtual void set_uint64(const char* key_name, std::uint64_t value) = 0;
...
struct ray_t {
	...
	ray_t(vec3_t const& start, vec3_t const& end) {
		...
		extents = vec3_aligned_t();

		world_axis_transform = NULL;
		is_ray = true;

		start_offset = vec3_aligned_t();
		this->start = start;
	}
...
public:
	...
	float ortho_right;
	float ortho_bottom;
	bool b_custom_view_matrix;
	matrix3x4_t custom_view_matrix;
	PAD(0x48);
	float fov;
	float fov_viewmodel;
	vec3_t origin;
...
public:
	...
	INLINE void dump_layer_to_debug() {
		DEBUG_LOG("~~~~~~~~~~~~~~~~~~~~~~\n");
		DEBUG_LOG("\nsequence: %d \n", sequence);
		DEBUG_LOG("weight: %f \n", weight);
		DEBUG_LOG("playback_rate: %f \n", playback_rate);
		DEBUG_LOG("cycle: %f \n", cycle);
		DEBUG_LOG("~~~~~~~~~~~~~~~~~~~~~~\n");
	}
};
...
struct clientside_animation_list_t {
	c_cs_player* player{};
	std::uint32_t flags{};
};
...
public:
	...
	bool walk_to_run_transition_state = 0;
